...
Matrix4x4 multiply_unrolled(const Matrix4x4 &A, const Matrix4x4 &B) {
  ...
  // Column 3
  C.m[12] = A.m[0] * B.m[12] + A.m[4] * B.m[13] + A.m[8] * B.m[14] +
            A.m[12] * B.m[15];
  C.m[13] = A.m[1] * B.m[12] + A.m[5] * B.m[13] + A.m[9] * B.m[14] +
            A.m[13] * B.m[15];
  C.m[14] = A.m[2] * B.m[12] + A.m[6] * B.m[13] + A.m[10] * B.m[14] +
            A.m[14] * B.m[15];
  C.m[15] = A.m[3] * B.m[12] + A.m[7] * B.m[13] + A.m[11] * B.m[14] +
            A.m[15] * B.m[15];
...
Matrix4x4 createPerspectiveProjectionMatrix(float fovY_deg, float aspect,
                       ...
                       const Vector3& world_up)
{
    ...
    M.m[12] = -dot_product(r, eye);
    M.m[13] = -dot_product(up, eye);
    M.m[14] =  dot_product(f, eye);
...
/**
 * @brief Multiplies a 3D vertex (treated as homogeneous coordinates) by a 4x4
 * matrix.
 *
...
Vector3 transform_vertex(const Matrix4x4 &M, const Vector3 &v) {
  ...
  if (std::abs(newW) > 1e-6f) {
    ...
    // Return the raw XYZ if W is near zero to prevent division by zero/large
    // errors.
